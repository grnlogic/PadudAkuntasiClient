          ...
          <div>
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
              <p className="text-yellow-800 text-sm font-semibold">
                ⚠️ PERHATIAN: Silakan cek dan ubah status absensi sesuai keadaan sebenarnya!
              </p>
            </div>
            ...
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-700">
              ...
              <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                <div><strong>✅ HADIR:</strong> Karyawan masuk kerja normal</div>
                <div><strong>🤒 SAKIT:</strong> Karyawan tidak masuk karena sakit</div>
                <div><strong>📝 IZIN:</strong> Karyawan tidak masuk dengan izin</div>
                <div><strong>❌ ALPA:</strong> Karyawan tidak masuk tanpa keterangan</div>
                <div><strong>🚫 OFF:</strong> Tidak ada jalur/tidak ada pekerjaan</div>
                <div><strong>½ Hari:</strong> Masuk tapi hanya dihitung setengah gaji</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4">
          ...
          <div className="bg-white p-3 rounded border">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-sm">
              <div>✅ Hadir: {filteredKaryawan.filter(k => absensi[k.id]?.status === "HADIR").length}</div>
              <div>🤒 Sakit: {filteredKaryawan.filter(k => absensi[k.id]?.status === "SAKIT").length}</div>
              <div>📝 Izin: {filteredKaryawan.filter(k => absensi[k.id]?.status === "IZIN").length}</div>
              <div>❌ Alpha: {filteredKaryawan.filter(k => absensi[k.id]?.status === "ALPA").length}</div>
              <div>🚫 Off: {filteredKaryawan.filter(k => absensi[k.id]?.status === "OFF").length}</div>
            </div>
          </div>
